...
class RoomSession:
    """单个讨论房间的异步会话。每个房间一个实例，随 SSE 连接生命周期存在。"""
    ...
    async def run(self) -> AsyncGenerator[dict, None]:
        """并行圆桌模式：每轮所有 agent 同时调用，谁先回来谁先显示。"""
        ...
        while round_num < max_rounds and not self._stop_event.is_set():
            ...
            async def call_one(a_id: str, a: AgentProfile) -> tuple:
                prompt = _AGENT_PROMPT.format(
                    system_prompt=a.system_prompt(),
                    topic=room.topic,
                    goal=room.goal or "自由讨论",
    ...
    def _save_msg(self, agent_id: str, agent_name: str, message_type: str,
                  content: str, *, avatar: str = "",
                  tool_name: str | None = None,
                  tool_input: str | None = None,
                  tool_output: str | None = None) -> Message:
        self._seq += 1
        msg = Message(
            room_id=self.room_id,
            seq=self._seq,
